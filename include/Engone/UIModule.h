...
namespace engone {
    ...
    struct UIModule;
    struct UILayout {
        UILayout(engone::UIModule* ui, engone::LinearAllocator* stringAllocator) : ui(ui), stringAllocator(stringAllocator) {} 
        float x=0, y=0;
        float textSize = 18;
        ...
        engone::UIModule* ui=nullptr;
        engone::LinearAllocator* stringAllocator = nullptr;

        UIText* makeText(const char* str, int length);
        UIBox* makeBox(float w, float h);
        UIBox* makeButton(const char* str, int length);
    };
    struct UITexture{
        unsigned int id;
        int width,height;
