...
void UIModule::setString(UIText* text, const char* string, int length) {
    Assert(text);
    Assert(string);
    Assert(text->id == 0); // strings for maintained text is more complicated
...
UIText* UILayout::makeText(const char* str, int length) {
    Assert(ui);
    Assert(stringAllocator);
